...
const Typewriter: React.FC<Props> = ({
  ...
  useEffect(() => {
    ...
    const jitter = (base: number) => Math.max(35, Math.round(base + (Math.random() - 0.5) * base));

    if (!isDeleting) {
      if (text.length < current.length) {
        timer = setTimeout(() => setText(current.slice(0, text.length + 1)), jitter(typingSpeed));
      } else {
        // schedule deleting after pause; because stableStrings is memoized, this timer will not be cleared by incidental renders
        timer = setTimeout(() => setIsDeleting(true), pause + Math.round(Math.random() * 600));
      }
    } else {
      if (text.length > 0) {
        timer = setTimeout(() => setText(current.slice(0, text.length - 1)), jitter(deletingSpeed));
      } else {
        timer = setTimeout(() => {
          setIsDeleting(false);
  ...
  return (
    <div className={`flex items-center justify-center min-h-[4rem] md:min-h-[5rem]`} style={{ overflow: 'visible' }}>
      <span className={`${className} inline-block`} style={{ lineHeight: '1.5' }} aria-live="polite">{text}</span>
      <span
        className={`ml-2 inline-block text-black dark:text-white font-heading text-4xl md:text-5xl font-normal`}
